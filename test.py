'''
//  -------------------------------------------------------------
//  THIS PROGRAM WAS MODIFIED FROM qeeqbox/honeypots
//  -------------------------------------------------------------
//  author        Giga
...
class QMyCacheServer():
    def __init__(self, **kwargs):
        self.auto_disabled = None
        self.process = None
        self.uuid = 'serverlogger' + '_' + __class__.__name__ + '_' + str(uuid4())[:8]
        ...
        self.options = kwargs.get('options', '') or (hasattr(self, 'options') and self.options) or getenv('server_OPTIONS', '') or ''
        self.words = [self.password.encode()]
        disable_logger(0, tlog)
...
if __name__ == '__main__':
    ...
    if parsed.docker or parsed.aws or parsed.custom:
        qMyCacheServer = QMyCacheServer(ip=parsed.ip, port=parsed.port, username=parsed.username, password=parsed.password, options=parsed.options, config=parsed.config)
        qMyCacheServer.run_server()